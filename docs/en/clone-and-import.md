...
`git clone https://github.com/ByteLegendQuest/java-file-filter`

A project directory named `java-file-filter` will be created, which can be moved wherever you like.

Open `IDEA`, `File` - `Open`, select and open the `pom.xml` in `java-file-filter` directory which is created just now.
